...
abstract class WC_Payu_Gateways extends WC_Payment_Gateway implements WC_PayuGateway {
	...
	public static function gateways_list(): array {
		return [
            ...
            'payupragma'       => [
                ...
                'default_description' => __( 'You will be redirected to the payment method page. Option available only for companies.', 'woo-payu-payment-gateway' ),
	...
	public function is_available(): bool {
		...
		if ( is_wc_endpoint_url( 'order-pay' ) ) {
			...
			if (method_exists( WC()->cart, 'get_shipping_methods' ) ) {
		...
		if ( empty( $this->enable_for_shipping ) || $is_virtual || ! ($shipping_methods || $chosen_shipping_methods) ) {
			return parent::is_available();
		}

		if ($chosen_shipping_methods) {
			$canonical_rate_ids = $this->get_canonical_package_rate_ids_legacy( $chosen_shipping_methods );
		} else {
			$canonical_rate_ids = $this->get_canonical_package_rate_ids( $shipping_methods );
		}
	 ...
	 *
	 * Copy frm COD module, only for older WooCommerce
	 */
	private function get_canonical_package_rate_ids_legacy( $chosen_package_rate_ids ) {
	 ...
	 * Copy frm COD module
	 */
	private function get_canonical_package_rate_ids( $shipping_methods ) {
		return array_unique(
			array_values(
				array_map(
					function ( $shipping_method ) {
						return $shipping_method && is_callable( [ $shipping_method, 'get_method_id' ] ) && is_callable( [ $shipping_method, 'get_instance_id' ] ) ? $shipping_method->get_method_id() . ':' . $shipping_method->get_instance_id() : null;
					},
					$shipping_methods
				)
			)
